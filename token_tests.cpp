...
TEST_CASE("Test tokenize A string expr", "[token]") {
  std::string input = "(\"aas df\" )";
  ...
  REQUIRE(tokens.front().type() == Token::STRING);
  REQUIRE(tokens.front().asString() == "\"aas df\"");
  

}

TEST_CASE("Test messing up tokenizeing A string expr", "[token]") {
  std::string input = "(\"aas df )";
